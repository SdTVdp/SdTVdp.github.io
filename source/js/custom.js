...
(() => {
    ...
    let ambientLayerState = null;
    let siteBackdropLayer = null;
    const siteBackdropProbeCache = new Map();
    ...
    const blendSeedSample = (foreground, background) => {
    ...
    const interpolateSampledColor = (from, to, ratio) => {
    ...
    const sampleSiteBackdropVisibleColor = async (event) => {
        ...
        const resolvedImage = imageOpacity >= 0.999
    ...
    const getColorDistance = (left, right) => Math.hypot(left.r - right.r, left.g - right.g, left.b - right.b);
    const getThemeBackdropColor = () => parseColor(window.getComputedStyle(document.documentElement).getPropertyValue("--global-bg")) ||
        parseColor(window.getComputedStyle(document.body).backgroundColor) ||
        { r: 31, g: 30, b: 51, a: 1 };
    const getEffectiveBackgroundColor = (element) => {
        ...
        return layers.reduce((resolved, layer) => {
            if (layer.a >= 0.999) {
    ...
    const chooseReadableColor = (foreground, background) => {
    ...
    const getAdaptiveContrastTargets = (root = document) => {
        if (!("querySelectorAll" in root)) {
            return [];
        }
        const selector = [
        ...
        return Array.from(root.querySelectorAll(selector)).filter((element, index, list) => {
    ...
    const rememberAdaptiveColorState = (element) => {
    ...
    const restoreAdaptiveColorState = (element) => {
        if (element.dataset.sdtvdpColorCaptured !== "true") {
            return;
        }
    ...
    const adaptInlineTextContrast = (root = document) => {
    ...
    const getAmbientSeededRandom = (salt = 0) => createSeededRandom((Date.now() ^ Math.floor(performance.now() * 1000) ^ Math.floor(window.scrollY) ^ salt) >>> 0);
    const getNormalizedPathname = () => {
        const pathname = window.location.pathname || "/";
        return pathname.endsWith("/") ? pathname : `${pathname}/`;
    };
    const isBackdropSurfacePage = () => {
        const pathname = getNormalizedPathname();
        const exactMatches = new Set(["/", "/about/"]);
        if (exactMatches.has(pathname)) {
            return true;
        }
        return ["/blog/", "/posts/", "/archives/", "/tags/", "/categories/"].some((prefix) => pathname === prefix || pathname.startsWith(prefix));
    };
    const getSiteBackdropCandidates = () => {
    ...
    const probeImageExists = (source) => new Promise((resolve) => {
        const image = new Image();
    ...
    const resolveSiteBackdropUrl = async () => {
    ...
    const resizeAmbientCanvas = (state) => {
    ...
    const ensureSiteBackdropLayer = async () => {
    ...
    const spawnAmbientMeteor = (state, now) => {
    ...
    const spawnAmbientSunbeam = (state, now) => {
    ...
    const drawAmbientSunbeam = (context, sunbeam) => {
    ...
    const renderAmbientEffects = (state) => {
        const width = state.canvas.width / state.pixelRatio;
        const height = state.canvas.height / state.pixelRatio;
    ...
    const tickAmbientEffects = (timestamp) => {
        if (!ambientLayerState) {
            return;
        }
    ...
    const ensureAmbientEffectsLayer = () => {
        if (prefersReducedMotion()) {
            removeAmbientLayer();
            return;
        }
