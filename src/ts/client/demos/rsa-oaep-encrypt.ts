(() => {
  ...
  const setPending = (button: HTMLButtonElement, output: HTMLElement, message: string): void => {
    button.disabled = true;
    output.textContent = message;
  };

  const restoreButton = (button: HTMLButtonElement): void => {
  ...
  const bootAll = (): void => {
    document
      .querySelectorAll<HTMLElement>('.crypto-demo[data-demo="rsa-oaep-encrypt"]')
