(() => {
  ...
  const readParams = (root: HTMLElement): RsaParams => {
  ...
  const computeRsa = (params: RsaParams): RsaResult => {
  ...
  const boot = (root: HTMLElement): void => {
    ...
    let result: RsaResult | null = null;
    let currentStep = 0;
  ...
  const bootAll = (): void => {
    document
      .querySelectorAll<HTMLElement>('.crypto-demo[data-demo="rsa-textbook"]')
