(() => {
  ...
  type ElGamalResult = ElGamalParams & {
    y: number;
    c1: number;
    sharedMask: number;
    c2M1: number;
    ...
    givenShared: number;
    givenInverse: number;
    decryptedGiven: number;
    steps: string[];
  };

  const DEFAULTS: ElGamalParams = {
    p: 509,
    g: 449,
    x: 12,
    k: 18,
  ...
  const modInverse = (value: number, modulus: number): number | null => {
  ...
  const getUniquePrimeFactors = (value: number): number[] => {
  ...
  const isGenerator = (g: number, p: number): boolean => {
    if (g <= 1 || g >= p) {
      return false;
    }
  ...
  const parseInteger = (input: HTMLInputElement, label: string): number => {
  ...
  const readParams = (root: HTMLElement): ElGamalParams => {
    const getInput = (role: string): HTMLInputElement => {
      const input = root.querySelector<HTMLInputElement>(`[data-role="${role}"]`);
  ...
  const computeElGamal = (params: ElGamalParams): ElGamalResult => {
  ...
  const boot = (root: HTMLElement): void => {
    ...
    let result: ElGamalResult | null = null;
    let currentStep = 0;
  ...
  const bootAll = (): void => {
    document
      .querySelectorAll<HTMLElement>('.crypto-demo[data-demo="elgamal-encrypt"]')
