...
(() => {
    ...
    const ADAPTERS = {
        ...
        "rsa-oaep-aes-gcm": {
            ...
            run: async (message) => {
                ...
                const wrappedKey = await crypto.subtle.encrypt({ name: "RSA-OAEP" }, rsaKeyPair.publicKey, rawAesKey);
                const restoredRawKey = await crypto.subtle.decrypt({ name: "RSA-OAEP" }, rsaKeyPair.privateKey, wrappedKey);
                const restoredAesKey = await crypto.subtle.importKey("raw", restoredRawKey, { name: "AES-GCM" }, false, ["decrypt"]);
                const restoredPlaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, restoredAesKey, ciphertext);
                return {
                    summaryLines: [
    ...
    const bootAll = () => {
        document
            .querySelectorAll('.crypto-demo[data-demo="hybrid-encryption"]')
