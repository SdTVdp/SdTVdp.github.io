...
- 当前活动主题是 **Butterfly / Pug**，已移除站点运行所需的 `EJS` 渲染器依赖
...
- 你现在的站点覆盖模板也是 Pug
- 根配置 `_config.yml` 当前使用 `theme: butterfly`
- `themes/minimal-cascade/` 里仍保留一套 EJS 旧版/实验主题模板，但它不是当前活动主题

所以当前实际运行的模板层只有一条主线。维护主题时优先看 Butterfly 配置和根目录 `layout/` 覆盖层；只有明确切换到 `minimal-cascade` 时，才需要维护那套 EJS 模板。
